...
class TestMPOWERDescriptives:
    ...
    def test_treatment_adoption_timeline(self, sample_data):
        ...
        timeline = descriptives.plot_treatment_adoption_timeline()
    ...
    def test_mpower_score_distribution(self, sample_data):
        ...
        distribution = descriptives.plot_mpower_score_distributions()
    ...
    def test_check_treatment_balance(self, sample_data):
        """Test treatment balance checking via summary statistics."""
        descriptives = MPOWERDescriptives(
            data=sample_data,
            country_col="country",
            year_col="year",
        ...
        # Test that summary statistics includes relevant information
        ...
        assert summary["dataset_structure"]["n_countries"] > 0
        assert summary["dataset_structure"]["n_observations"] > 0

        # Test that plotting method works (treatment balance visualization)
        fig = descriptives.plot_treatment_balance_check()
        assert fig is not None

    def test_correlation_analysis(self, sample_data):
        """Test correlation analysis via plot correlation heatmap."""
        descriptives = MPOWERDescriptives(
            data=sample_data,
            country_col="country",
            year_col="year",
        ...
        fig = descriptives.plot_correlation_heatmap(variables=variables)
        assert fig is not None

        # Test with default variables (should work without specifying)
        fig_default = descriptives.plot_correlation_heatmap()
