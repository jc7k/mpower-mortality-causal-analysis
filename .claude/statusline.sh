...
# Custom Claude Code statusline - Created: 2025-08-17T18:34:46.028Z
...
# ---- basics ----
# Find jq in common locations
JQ_CMD=""
for jq_path in jq /usr/bin/jq /usr/local/bin/jq "$HOME/bin/jq"; do
...
if [ -n "$JQ_CMD" ]; then
  current_dir=$(echo "$input" | "$JQ_CMD" -r '.workspace.current_dir // .cwd // "unknown"' 2>/dev/null | sed "s|^$HOME|~|g")
  model_name=$(echo "$input" | "$JQ_CMD" -r '.model.display_name // "Claude"' 2>/dev/null)
  model_version=$(echo "$input" | "$JQ_CMD" -r '.model.version // ""' 2>/dev/null)
else
  current_dir="unknown"
...
if [ -n "$JQ_CMD" ]; then
  ...
  if [ -n "$blocks_output" ]; then
    active_block=$(echo "$blocks_output" | "$JQ_CMD" -c '.blocks[] | select(.isActive == true)' 2>/dev/null | head -n1)
    if [ -n "$active_block" ]; then
      cost_usd=$(echo "$active_block" | "$JQ_CMD" -r '.costUSD // empty')
      cost_per_hour=$(echo "$active_block" | "$JQ_CMD" -r '.burnRate.costPerHour // empty')
      tot_tokens=$(echo "$active_block" | "$JQ_CMD" -r '.totalTokens // empty')
      tpm=$(echo "$active_block" | "$JQ_CMD" -r '.burnRate.tokensPerMinute // empty')

      # Session time calculation
      reset_time_str=$(echo "$active_block" | "$JQ_CMD" -r '.usageLimitResetTime // .endTime // empty')
      start_time_str=$(echo "$active_block" | "$JQ_CMD" -r '.startTime // empty')
