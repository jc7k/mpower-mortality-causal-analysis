...
def debug_event_study_data():
    ...
    print("Data types in pipeline data:")
    ...
    for var in pipeline.control_vars:
        if var in pipeline.data.columns:
            print(
                f"{var}: {pipeline.data[var].dtype}, has NaN: {pipeline.data[var].isna().any()}"
            )
    ...
    print(
        f"{outcome}: {pipeline.data[outcome].dtype}, has NaN: {pipeline.data[outcome].isna().any()}"
    )
    ...
    print(
        f"Event time unique values: {sorted(event_study.data['event_time'].unique())[:10]}"
    )
    ...
    for var in test_vars:
        if var in data_with_dummies.columns:
            ...
            try:
                ...
                print("    -> Can convert to numeric: YES")
            except:
                print("    -> Can convert to numeric: NO")
