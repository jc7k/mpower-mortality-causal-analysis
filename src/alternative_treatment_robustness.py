...
def create_binary_treatment(
    data: pd.DataFrame, threshold: float, min_periods: int = 2
) -> pd.DataFrame:
    ...
    for country in data["country_name"].unique():
        ...
        high_periods = (
            country_data["mpower_high"]
            .rolling(window=min_periods, min_periods=min_periods)
            .sum()
        )
        ...
        if sustained_high.any():
            ...
            mask = (data["country_name"] == country) & (
                data["year"] >= first_sustained_year
            )
    ...
    for threshold in thresholds:
        ...
        for outcome in OUTCOMES:
            result = run_twfe_regression(
                binary_data, "post_treatment", outcome, DEFAULT_CONTROL_VARS
            )
...
def improvement_robustness_analysis(data: pd.DataFrame) -> dict:
    ...
    data_with_baseline["baseline_mpower"] = data_with_baseline["country_name"].map(
        baseline_scores
    )
    ...
    for improvement_level in improvement_levels:
        ...
        for outcome in OUTCOMES:
            result = run_twfe_regression(
                improvement_data, "post_improvement", outcome, DEFAULT_CONTROL_VARS
            )
...
def control_variables_robustness(data: pd.DataFrame) -> dict:
    ...
    for spec_name, controls in control_specs.items():
        ...
        for outcome in OUTCOMES:
            result = run_twfe_regression(
                binary_data, "post_treatment", outcome, controls
            )
...
def sample_restrictions_analysis(data: pd.DataFrame) -> dict:
    ...
    for outcome in OUTCOMES:
        result = run_twfe_regression(
            high_income_binary, "post_treatment", outcome, DEFAULT_CONTROL_VARS
        )
    ...
    for outcome in OUTCOMES:
        result = run_twfe_regression(
            complete_binary, "post_treatment", outcome, DEFAULT_CONTROL_VARS
        )
    ...
    for outcome in OUTCOMES:
        result = run_twfe_regression(
            recent_binary, "post_treatment", outcome, DEFAULT_CONTROL_VARS
        )
...
def placebo_tests(data: pd.DataFrame) -> dict:
    ...
    for country, placebo_year in placebo_treatment.items():
        if (
            placebo_year >= placebo_data["year"].min()
        ):  # Only if placebo year is in data
            mask = (placebo_data["country_name"] == country) & (
                placebo_data["year"] >= placebo_year
            )
    ...
    for outcome in OUTCOMES:
        result = run_twfe_regression(
            placebo_data, "placebo_post", outcome, DEFAULT_CONTROL_VARS
        )
...
def create_robustness_summary(results: dict, output_dir: Path):
    ...
    if not threshold_df.empty:
        threshold_df.to_excel(
            output_dir / "threshold_robustness_summary.xlsx", index=False
        )
    ...
    if not controls_df.empty:
        controls_df.to_excel(
            output_dir / "controls_robustness_summary.xlsx", index=False
        )
    ...
    if not threshold_df.empty:
        for outcome in OUTCOMES:
            ...
            if not outcome_data.empty:
                ...
                print(
                    f"   {outcome}: {sig_count}/{total_count} significant across thresholds"
                )
    ...
    if not controls_df.empty:
        for outcome in OUTCOMES:
            ...
            if not outcome_data.empty:
                ...
                total_count = len(outcome_data)
                print(
                    f"   {outcome}: {sig_count}/{total_count} significant across specifications"
                )
    ...
    print(
        "- Placebo tests: Should show no significant effects if identification is valid"
    )
